...
inst_structure_server <- function(id, db_pool, user_data) {
  moduleServer(id, function(input, output, session) {
    ...
    observeEvent(input$save_unit_btn, {
      data <- list(
        name = input$new_unit_name,
        unit_type = input$new_unit_type
      )
...
inst_resources_server <- function(id, db_pool, user_data) {
  moduleServer(id, function(input, output, session) {
    ...
    observeEvent(input$save_resource_btn, {
      data <- list(
        name = input$new_res_name,
        resource_type = input$new_res_type,
        unit_id = input$new_res_unit,
        budget_amount = input$new_res_budget
      )
      ...
      tryCatch({
        ...
        ", params = list(
          ...
          input$new_res_desc, input$new_res_qty, input$new_res_budget,
          input$new_res_condition
        ))
        removeModal()
        load_resources()
...
inst_contingent_server <- function(id, db_pool, user_data) {
  moduleServer(id, function(input, output, session) {
    ...
    output$personnel_by_type_chart <- renderPlotly({
      ...
      by_type <- data %>%
        group_by(personnel_type) %>%
        summarise(count = n(), .groups = "drop")

      labels <- sapply(by_type$personnel_type, function(x) {
        lbl <- PERSONNEL_TYPES[x]; if(is.na(lbl)) x else lbl
      })
    ...
    observeEvent(input$save_personnel_btn, {
      data <- list(
        full_name = input$new_pers_name,
        personnel_type = input$new_pers_type,
        unit_id = input$new_pers_unit
      )
...
inst_monitoring_server <- function(id, db_pool, user_data) {
  moduleServer(id, function(input, output, session) {
    ...
    observe({
      ...
      areas <- areas_data()
      if (nrow(areas) > 0) {
        updateSelectInput(session, "kpi_activity_filter",
          choices = c("Hamısı" = "", setNames(areas$id, areas$name)))
      }

      kpis <- kpis_data()
      if (nrow(kpis) > 0) {
        updateSelectInput(session, "prog_kpi_filter",
          choices = c("Hamısı" = "", setNames(kpis$id, kpis$name)))
      }
    })
    ...
    output$kpis_table <- renderDT({
      ...
      display_data <- data.frame(
        Ad = data$name,
        `Fəaliyyət` = data$activity_name,
        `Hədəf` = data$target_value,
        `Baza` = data$baseline_value,
    ...
    output$kpi_completion_chart <- renderPlotly({
      ...
      plot_ly(completion, x = ~name, y = ~completion_pct, type = "bar",
        ...
        text = ~paste0(completion_pct, "%"), textposition = "outside") %>%
        layout(
          xaxis = list(title = "", tickangle = -45),
          yaxis = list(title = "Tamamlanma (%)", range = c(0, max(120, max(completion$completion_pct) + 10))),
          shapes = list(
            list(type = "line", x0 = -0.5, x1 = nrow(completion) - 0.5,
    ...
    output$projects_table <- renderDT({
      ...
      display_data <- data.frame(
        ...
        `Başlama` = format(as.Date(data$start_date), "%d.%m.%Y"),
        check.names = FALSE
      )
    ...
    output$progress_table <- renderDT({
      ...
      display_data <- data.frame(
        KPI = data$kpi_name,
        `Faktiki` = data$actual_value,
        `Hədəf` = data$target_value,
        `Ölçü` = data$unit_measure,
        `Dövr sonu` = format(as.Date(data$period_end), "%d.%m.%Y"),
        Qeyd = data$notes,
        check.names = FALSE
      )
    ...
    output$progress_trend_chart <- renderPlotly({
      ...
      plot_ly(data, x = ~period_end, y = ~actual_value, type = "scatter", mode = "lines+markers",
        ...
        add_trace(y = ~target_value, name = "Hədəf",
          ...
          marker = list(size = 0)) %>%
        layout(
          xaxis = list(title = "Tarix"),
          yaxis = list(title = "Dəyər"),
          hovermode = "x unified"
        )
    })
    ...
    observeEvent(input$add_area_btn, {
      ...
      showModal(modalDialog(
        ...
        footer = tagList(
          modalButton("Ləğv et"),
          actionButton(ns("save_area_btn"), "Saxla", class = "btn-primary")
        ),
        size = "m"
      ))
    })
    ...
    observeEvent(input$save_area_btn, {
      tryCatch({
        dbExecute(db_pool, "
          INSERT INTO activity_areas (unit_id, name, category, description, priority)
          VALUES ($1, $2, $3, $4, $5)
        ", params = list(
          input$new_area_unit, input$new_area_name, input$new_area_category,
          input$new_area_desc, input$new_area_priority
        ))
        removeModal()
        load_areas()
        showNotification("Fəaliyyət sahəsi uğurla əlavə edildi", type = "message")
      }, error = function(e) {
        log_error("Fəaliyyət sahəsi əlavə xətası: {e$message}")
        showNotification("Xəta baş verdi", type = "error")
      })
    })
    ...
    observeEvent(input$add_kpi_btn, {
      ...
      showModal(modalDialog(
        ...
        footer = tagList(
          modalButton("Ləğv et"),
          actionButton(ns("save_kpi_btn"), "Saxla", class = "btn-primary")
        ),
        size = "m"
      ))
    })
    ...
    observeEvent(input$save_kpi_btn, {
      data <- list(
        name = input$new_kpi_name,
        target_value = input$new_kpi_target,
        activity_id = input$new_kpi_activity
      )
      ...
      tryCatch({
        dbExecute(db_pool, "
          INSERT INTO activity_kpis (activity_id, name, description, unit_measure, target_value, baseline_value, frequency)
          VALUES ($1, $2, $3, $4, $5, $6, $7)
        ", params = list(
          input$new_kpi_activity, input$new_kpi_name, input$new_kpi_desc,
          input$new_kpi_unit, input$new_kpi_target, input$new_kpi_baseline,
          input$new_kpi_frequency
        ))
        removeModal()
        load_kpis()
        showNotification("KPI uğurla əlavə edildi", type = "message")
      }, error = function(e) {
        log_error("KPI əlavə xətası: {e$message}")
        showNotification("Xəta baş verdi", type = "error")
      })
    })

    # İrəliləyiş qeyd et modal
    observeEvent(input$add_progress_btn, {
      ...
      showModal(modalDialog(
        ...
        footer = tagList(
          modalButton("Ləğv et"),
          actionButton(ns("save_progress_btn"), "Saxla", class = "btn-primary")
        ),
        size = "m"
      ))
    })
    ...
    observeEvent(input$save_progress_btn, {
      tryCatch({
        dbExecute(db_pool, "
          INSERT INTO activity_progress (kpi_id, period_start, period_end, actual_value, notes)
          VALUES ($1, $2, $3, $4, $5)
        ", params = list(
          input$new_prog_kpi, as.character(input$new_prog_start),
          as.character(input$new_prog_end), input$new_prog_value, input$new_prog_notes
        ))
        removeModal()
        load_progress()
        showNotification("İrəliləyiş qeydi uğurla əlavə edildi", type = "message")
      }, error = function(e) {
        log_error("İrəliləyiş qeydi xətası: {e$message}")
        showNotification("Xəta baş verdi", type = "error")
      })
    })
    ...
    observeEvent(input$add_project_btn, {
      ...
      showModal(modalDialog(
        ...
        footer = tagList(
          modalButton("Ləğv et"),
          actionButton(ns("save_project_btn"), "Saxla", class = "btn-primary")
        ),
        size = "m"
      ))
    })
    ...
    observeEvent(input$save_project_btn, {
      data <- list(
        name = input$new_proj_name,
        unit_id = input$new_proj_unit,
        budget = input$new_proj_budget
      )
      ...
      tryCatch({
        dbExecute(db_pool, "
          INSERT INTO unit_projects (unit_id, name, description, budget, priority, status, start_date, end_date, manager_name)
          VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
        ", params = list(
          input$new_proj_unit, input$new_proj_name, input$new_proj_desc,
          input$new_proj_budget, input$new_proj_priority, input$new_proj_status,
          as.character(input$new_proj_start), as.character(input$new_proj_end),
          input$new_proj_manager
        ))
        removeModal()
        load_projects()
        showNotification("Layihə uğurla əlavə edildi", type = "message")
      }, error = function(e) {
        log_error("Layihə əlavə xətası: {e$message}")
        showNotification("Xəta baş verdi", type = "error")
      })
    })
  })
